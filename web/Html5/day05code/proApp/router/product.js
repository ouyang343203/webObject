...
/*********************开始写路由接口 请求方式get******************************/
// 1.定义一个查询所有商品信息的 接口名称：此处我们定义为/allList 请求方式为:get 没有请求参数
//测试地址:http://127.0.0.1:8080/v2/pro2/allList
...
router.get('/show2/:id',(req,res)=>{//这里使用id作为标记接口传递过来的id值
	...
	res.send(prodata[goodsid-1])
})


// 4.1定义删除一条商品信息的路由使用查询字符串参数传参 接口名称：此处我们定义为/del  请求方式为:DELRTE 参数:id=3  
//测试地址:http://127.0.0.1:8080/v2/pro2/del?id=3   注意这里的接口参数id 通过问好 方式补在后面的
//注释测试要在ApiPost工具中测试
...
// 测试地址:http://127.0.0.1:8080/v2/pro2/del2/3   注意这里的接口参数id 通过是/杆 方式补在后面的
// 注释测试要在ApiPost工具中测试
...
// 测试地址:http://127.0.0.1:8080/v2/pro2/add 
...
/*********************开始写路由接口 请求方式put******************************/
// 1定义修改一条商品信息的路由  接口名称：此处我们定义为/editor  请求方式为:put 参数:id=4 
// 测试地址:http://127.0.0.1:8080/v2/pro2/editor
// 注释测试要在ApiPost工具中测试方法POST ,application/x-www-form-urlencodeed
// 切记要在apiPost的body中写参数与参数值  id=11 tit=MacBook price=8999
// 相应消息问：新增的商品信息为:编号11,价格:8999，标题:MacBook
// 特别注意POST请求需要在server.js中设置一个中间
